...
const SIMULACROS = {
  1: {
    ...
    titulo: "SABER 11°",
    descripcion:
      "Configura tu examen - Selecciona cantidad de preguntas por asignatura",
    shortName: "Simulacro",
  ...
  2: {
    ...
    nombre: "Simulacro 2",
    titulo: "SABER 11°",
    descripcion: "Primer simulacro SED - Diagnóstico",
...
function getSubjectDefaultConfig(subject) {
  return (
    DEFAULT_SUBJECT_CONFIG[subject] || {
      preguntas: 3,
      inicio: 1,
      aleatorio: true,
    }
...
function validateSubjectConfig(subject, config, simulacroId = 1) {
  ...
  if (config.preguntas > maxQuestions) {
    errors.push(
      `Máximo ${maxQuestions} preguntas disponibles para ${SUBJ_INFO[subject]?.name || subject}`,
    );
  }
  if (config.inicio < 1) {
  ...
  if (config.inicio > maxQuestions) {
    errors.push(
      `El inicio no puede superar el total de preguntas (${maxQuestions})`,
    );
  }
  if (config.inicio + config.preguntas - 1 > maxQuestions) {
    errors.push(
      `El rango excede el total de preguntas disponibles (inicio ${config.inicio} + ${config.preguntas - 1} > ${maxQuestions})`,
    );
  }
...
const NIVEL_INFO = {
  lc: {
    ...
    2: {
      range: "Puntaje 36 – 50",
      summary:
        "Además de lo descrito en el nivel anterior, el estudiante que se ubica en este nivel comprende textos continuos y discontinuos de manera literal. Reconoce información explícita y la relaciona con el contexto. <br><br>Para clasificar en este nivel, el estudiante:",
      items: [
    ...
    3: {
      range: "Puntaje 51 – 65",
      summary:
        "Además de lo descrito en los niveles anteriores, el estudiante que se ubica en este nivel interpreta información de textos al inferir contenidos implícitos y reconocer estructuras, estrategias discursivas y juicios valorativos. <br><br>Para clasificar en este nivel, el estudiante:",
      items: [
  ...
  mat: {
    ...
    4: {
      ...
      summary:
        "Además de lo descrito en los niveles anteriores, el estudiante que se ubica en este nivel resuelve problemas y justifica la veracidad o falsedad de afirmaciones que requieren el uso de conceptos de probabilidad, propiedades algebraicas, relaciones trigonométricas y características de funciones reales. Lo anterior, en contextos principalmente matemáticos o científicos abstractos.<br><br>Para clasificar en este nivel, el estudiante:",
      items: [
  ...
  cn: {
    ...
    3: {
      range: "Puntaje 56 – 70",
      summary:
        "Además de lo descrito en los niveles anteriores, el estudiante que se ubica en este nivel interrelaciona conceptos, leyes y teorías científicas con información presentada en diversos contextos, en los que intervienen dos o más variables, para hacer inferencias sobre una situación problema o un fenómeno natural. <br><br>Para clasificar en este nivel, el estudiante:",
      items: [
